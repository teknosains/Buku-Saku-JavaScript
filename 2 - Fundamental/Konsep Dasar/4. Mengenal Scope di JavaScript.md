...
Scope adalah salah satu konsep fundamental yang **wajib** diketahui dan difahami sebelum lebih jauh belajar JavaScript. Scope sejatinya adalah _access_. Scope di JavaScript berbicara tentang lokasi dimana variable-variable dapat diakses atau tidak dapat diakses.
...
Adapun deklarasi variable dengan ```let``` dan ```const``` tetap sama-sama di hoisting oleh JavaScript engine. Hanya saja mereka punya behavior yang berbeda dengan ```var```.
...
Materi ini sejujurnya memang bisa cukup sulit dicerna oleh pemula. Mengingat begitu penting nya bagian ini, oleh karena itu baiknya dibaca lagi beberapa kali dan jika perlu dikombinasikan dengan sumber lainnya agar semakin faham.
